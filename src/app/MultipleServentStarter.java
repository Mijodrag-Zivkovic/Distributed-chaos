...
public class MultipleServentStarter {
	...
	private static void startServentTest(String testName) {
		...
		//proces builder za bootstrap

		try {
			ProcessBuilder bsBuilder = new ProcessBuilder("java", "-cp", "bin/", "app.BootStrapMain", String.valueOf(AppConfig.BS_PORT));
			bsBuilder.redirectOutput(new File(testName+"/output/bs_out.txt"));
			bsBuilder.redirectError(new File(testName+"/error/bs_err.txt"));
			Process bs = bsBuilder.start();
			serventProcesses.add(bs);
		} catch (IOException e) {
			e.printStackTrace();
		}
		// ne zaboravi da dodas u listu procesa
		...
		for (Process process : serventProcesses) {
			if (process.equals(serventProcesses.get(0)))
				continue;
			try {
	...
	public static void main(String[] args) {
		...
		//startServentTest("input7");
		startServentTest("input7quit");
	}
